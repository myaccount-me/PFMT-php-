...
$requete="SELECT * FROM laboratoire WHERE num_lab=$n";
//echo($requete); 
 ...
 ?> 
         ...
         <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                   ...
                   <div class="x_panel">
				 <div class="x_content"> 
				   <h2 style=" text-align:; text-transform: uppercase;  color:#23798E ;">Modifier un laboratoire</h2>
                <br/>
                 <br/>
				 
				 
              <form   class="form-horizontal form-label-left" action="../../control/updatelabo.php" method="post">
			    <?php if(isset($_GET['num'])){ ?>
						     <input type="hidden"  id='num' class="form-control   has-feedback-left"  name="num" required="required" style='border-radius: 5px;' value="<?php echo $_GET['num'] ;?>">
			<?php } else {?>			
                          <input type="hidden" id="num"  class="form-control " name="num" value="<?php echo($num_lab)?>" readonly="readonly">
					<?php } ?>	 
				 ...
				 <input type="text" class="form-control has-feedback-left" name="lab" id='lab' required="required" style='border-radius: 5px;' onKeyUp="myFunction()"value="<?php echo $_GET['lab'] ;?>" onKeyUp="myFunction()">
				  <span class="fas fa-bong form-control-feedback left" aria-hidden="true"></span>
				 <?php } else { ?>
				<input type="text" class="form-control has-feedback-left" name="lab" id='lab' required="required" style='border-radius: 5px;' value="<?php echo($nom_lab)?>" onKeyUp="myFunction()" >
				 <span class="fas fa-bong form-control-feedback left" aria-hidden="true"></span>
				  <?php } ?>
  ...
  x.value = x.value.toUpperCase();
                       ...
                       <div class="form-group row">
						...
						<?php } else { ?>
						<input type="email" class="form-control has-feedback-left" name="mail" required="required"  style='border-radius: 5px;' value="<?php echo($mail)?>">
						<span class="fa fa-envelope form-control-feedback left" aria-hidden="true"></span>
						<?php } ?>
					  ...
					  <div class="form-group row">
						 ...
						 <div class="col-sm-3">
						  ...
						  <?php } else { ?>
                          <input type="text"   class="form-control  has-feedback-left"  name="pays" required="required" style='border-radius: 5px;' value="<?php echo($pays)?>">
						<?php } ?>
					 ...
					 <?php } else { ?>
					     <input type="text"   class="form-control " name="ville" required="required" style='border-radius: 5px;' value="<?php echo($ville)?>">
					 <?php } ?>
			            ...
			            <label  class=" control-label   col-md-2 col-sm-2 col-xs-1  col-form-label" >Code Postal:<span class="required"></span></label>
							 <div class=" col-md-3 col-sm-1 col-xs-1">
							  ...
							  <?php } else { ?>
                        <input type="text"   class="form-control " name="cp" required="required" style='border-radius: 5px;' value="<?php echo($cp)?>">
						<?php } ?>
					   ...
					   <label  class=" control-label col-sm-2 col-form-label" >Adresse:<span class="required"></span> </label>
                        <div class="col-sm-6">
						 <?php if(isset($_GET['adresse'])){ ?>
						  ...
						  <?php } else { ?>
                            <input type="text"   class="form-control " name="adresse" required="required" style='border-radius: 5px;' value="<?php echo($adresse)?>">
						  <?php } ?>
                        ...
                        <div class="col-sm-3 has-feedback">
						 <?php if(isset($_GET['contact'])){ ?>
						  <input type="text"   class="form-control has-feedback-left " name="contact" required="required" style='border-radius: 5px;' value="<?php echo $_GET['contact'] ;?>">
						   <?php } else { ?>
                          <input type="text"   class="form-control  has-feedback-left " name="contact" required="required" style='border-radius: 5px;' value="<?php echo($contact)?>">
						  <?php } ?>
					  ...
					  <label class=" control-label col-sm-1 col-form-label" >T.Bureau:<span class="required"></span></label>
						...
						<?php if(isset($_GET['tel'])  &&  $_GET['tel']=="erreur"){ ?>
						  ...
						  <?php } else { ?>
                          <input type="tel"   class="form-control " name="tel" pattern="[0-9]{8}" required="required" style='border-radius: 5px;' value="<?php echo($tel)?>" >
						   <span class="fa fa-phone form-control-feedback right" aria-hidden="true"></span>
						    <?php } ?>
					   ...
					   <div class="form-group row ">
                        ...
                        <label class=" control-label col-sm-2 col-form-label" >Qualite du Laboratoire:<span class="required"></span></label>
						  ...
						  <?php } else { ?>
                     <input type="text"   class=" form-control form-control-lg" name="qualitelabo" required="required" style='border-radius: 5px;' value="<?php echo($qualite)?>" >
					      <?php } ?>
		...
		<a href="formlistelabo.php"><button type="button" class=" btnn success "  style='border-radius: 5px;'><i class="	fas fa-sync-alt"></i> Annuler</button></a>	 	
